...
#let slides(
  ...
  set page(
    ...
    width: width,
    height: height,
    margin: (x: 0.5 * space, top: space, bottom: 0.2 * space),
    // HEADER
    header: [
      #context {
        let page = here().page()
  ...
  // Terms
  show terms.item: it => {
    set block(width: 100%, inset: 5pt, above: 0.1cm, below: 0.1cm)
    stack(
...
// Key message
#let keymsg(body) = {
  set text(weight: "bold", fill: blue.darken(60%))
  body
