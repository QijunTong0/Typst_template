...
#definition[確率過程${X_t}_(0<=t<=1)$がBernstein過程であるとは前向きと後向きのフィルトレーション$cal(F)_t^+,cal(F)_t^-$に対して任意の$t_0<t_1<t_2$で$P(X_t_1 in A|cal(F)_(t_0)^+,cal(F)_(t_2)^-)=P(X_t in A|X_t_0,X_t_2)$を満たす確率過程である.]
例えば ウィーナー過程 $W_t$に対して$B_t:=(W_t|W_1=0)$はbrownian bridgeと呼ばれるベルンシュタイン過程の簡単な例である.
== マルコフ性を持つBernstein過程
...
#theorem[
  ...
  $
    q(x,t_1;z,t_2;y,t_3):= (g(x,t_1-t_2,z)g(z,t_2-t_3,y))/g(x,t_3-t_1,y) \
    Q(x,t;F,r;y,s)=integral q(x,t;z,r;y,s) d z"と表される."
  $
]
== Schrödinger Bridgeとエントロピー正則化最適輸送
Schrödinger Bridge(SB)はマルコフ性を持つBernstein過程の重要な例である.SBの定義は多数あるがここでは次のように定義する.
#definition[
  連続なPath space$Omega=C([0,1],X)$上の確率測度全体$cal(P)(Omega)$と$R in cal(P)(Omega)$を考える.このとき,
  $
    inf_(P in cal(P)(Omega)) "KL"(P|R), "subject to" P_0=mu,P_1=nu
  $
  の解に対応する確率過程をSchrödinger Bridgeという.
]
SBはBrownian bridgeをエントロピー正則化最適輸送問題の解で混合したものとして表現できることが知られている.
#theorem[
  $pi$をエントロピー正則化最適輸送
]
...
Benrstein過程は始点と終点の分布を確率過程の意味で補間するような汎用的な問題設定であり、応用も補間に関わるものが多い.
生成モデルでの活用では
