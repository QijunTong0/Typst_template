...
本稿では、3つの核心的な公理に基づいた #emph[公理的量子物理学] について簡潔に紹介する。

= はじめに

まず、物理系は「状態」と呼ばれる数学的対象と、通常は微分方程式によって支配されるその「時間発展」によって記述されることを想起しよう。古典物理学において、状態は一般化座標 $q=(q_1,...,q_f)$ とそれに対応する一般化運動量 $p=(p_1,...,p_f)$ によって定義される。そして、時間発展は *ハミルトン関数* および #emph[ハミルトンの正準方程式] によって決定される。量子系の状態の定義を、以下の公理として与える。
...
量子物理学において、状態 $psi in cal(H)$ における観測可能量 $T$ は確率的である。そこで、確率変数 $T_psi$ を定義する。ここで、確率空間 $(Omega,cal(B)(bb(R)),P)$ 上の $T_psi$ に関する公理を追加する。
...
#proof[
  ...
  $
    E[T_psi] & = integral_bb(R) lambda d P_T_psi (lambda) \
